...
function openpage(){
    console.log( "login!" );
    let newurl = "/index.html?"+username+"&"+password;
...
function acountCheck( user, pass, f1 ){
    ...
    req.open("GET","https://pacific-scrubland-78772.herokuapp.com/ac2.php?user="+user+"&pass="+pass );
...
function createAcount( user, pass, f1 ){
    let req = new XMLHttpRequest();
    req.open("GET","https://pacific-scrubland-78772.herokuapp.com/acCre2.php?user="+user+"&pass="+pass );
    req.onreadystatechange = function(){
        if(req.readyState == 4 && req.status == 200 ){
            console.log( "user", user, "pass", pass );
            let res = req.responseText;
